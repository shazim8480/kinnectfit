...
//             className={`flex selectable justify-between items-center rounded-lg shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] ${(selected && accessToken) ? 'border-2 border-blue-800 rounded-sm' : ''
//                 } ${(alreadySelected && accessToken) ? 'opacity-[0.6] border-2 rounded-sm border-blue-800' : ''
//                 }`}
//         >
...
const MealComponent = ({ meal, selected, onClick, alreadySelected, setSelectedMeals, setSelectedMealStates }) => {
    const accessToken = getItemFromLocalStorage('accessToken');
 

    const isSelected = selected && accessToken;
    const isAlreadySelected = alreadySelected && accessToken;
    ...
    return (
        <div
            ...
            className={`flex selectable justify-between items-center rounded-lg shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] ${selected ? 'border-2 border-blue-800 rounded-sm' : ''
                } ${isAlreadySelected ? 'opacity-[0.6] border-2 rounded-sm border-blue-800' : ''
                }`}
        >
            <div className="flex flex-col max-w-[14rem] md:max-w-lg pl-0 md:pl-8">
                <div className="flex justify-between">
